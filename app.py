from data_processing.parser import parse_job
from data_processing.processor import get_key_words, flatten, merge_entries, process_dataframe, clean_text
...
async def main():
    st.title(":male-technologist: Job Insight Analyzer")
    st.write("Добро пожаловать в Job Insight Analyzer – ваш персональный помощник для анализа вакансий на hh.ru! Это инструмент, который преобразует поиски работы в наглядные данные.")
    ...
    if find_job_button:
        ...
        with st.spinner("Выполняется поиск вакансий…"):
            ...
            for skill_level in df['experience_name'].unique():
                requirements = df[df['experience_name'] == skill_level]['snippet_requirement'].dropna().apply(clean_text).tolist()
            ...
            for skill_level in df['experience_name'].unique():
                responsibility = df[df['experience_name'] == skill_level]['snippet_responsibility'].dropna().apply(clean_text).tolist()
